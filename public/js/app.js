...
function loadBanner() {
    ...
    if (bannerUrl && isValidImageUrl(bannerUrl)) {
        ...
        // Add error handling in case URL is bad
        bannerImage.onerror = function() {
            this.src = 'https://placehold.co/800x300/2e7d32/ffffff?text=AgroMart+Special+Offers';
        };
    }
}
    ...
    return `
        <div class="product-card" data-product-id="${product.id}">
            <div class="product-image">
                <img src="${escapeHTML(product.image)}" alt="${escapeHTML(product.name)}" loading="lazy" onerror="this.src='https://placehold.co/300x300/cccccc/666666?text=No+Image'">
            ...
            `<span class="original-price" id="detailOriginalPrice">₹${currentVariant ? currentVariant.originalPrice : product.originalPrice}</span>` : ''}
                    ...
                    ` : ''}
                     ...
                     <div class="shipping-info" style="margin-top: 20px;">
                        ...
                        <div class="shipping-subtext">For bulk quantity: <a href="https://wa.me/${escapeHTML(product.bulkOrderNumber || getWhatsAppNumber())}?text=${encodeURIComponent(`Hello, I want to order ${product.name} in bulk.`)}" class="whatsapp-link" target="_blank">${escapeHTML(product.bulkOrderNumber || getWhatsAppNumber())}</a></div>
                    </div>
                </div>
            </div>
        </div>
...
window.addToCartFromDetail = function () {
    ...
    if (currentProduct.variants && currentProduct.variants.length > 0) {
        variantIndex = currentProduct.variants.indexOf(currentVariant);
        if (variantIndex === -1) variantIndex = 0; // Fallback to first variant
    }
...
function handleWhatsAppOrder() {
    ...
    if (!cooldown.allowed) {
        showToast(`Limit reached. Please wait ${cooldown.remaining} ${cooldown.unit} before sending another message.`, 'error');
        return;
    }
...
function loadProductDetails(productId) {
    ...
    container.innerHTML = `
        ...
        <div class="product-gallery">
            <div class="main-image-container">
                <img id="detailMainImage" src="${allImages[0]}" alt="${product.name}" onclick="stopImageSlider();" ontouchstart="stopImageSlider();" onerror="this.src='https://placehold.co/500x500/cccccc/666666?text=No+Image'">
            ...
            <div class="thumbnail-list">
                ${allImages.map((img, idx) => `
                    <div class="thumbnail ${idx === 0 ? 'active' : ''}" onclick="updateMainImage('${img}', this)">
                        <img src="${img}" alt="Thumb" onerror="this.src='https://placehold.co/80x80/cccccc/666666?text=No'">
                    </div>
                `).join('')}
            </div>
        </div>
