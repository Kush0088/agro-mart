...
async function withRetry(fn, retries = 3, delay = 1000) {
    for (let i = 0; i < retries; i++) {
        try {
            ...
            console.warn(`⚠️ [Sheets] Operation failed, retrying (${i + 1}/${retries})...`, err.message);
...
function updateConfig(newConfig) {
    // ⚡ SECURITY FIX: Don't log key length or presence in production
    ...
    console.log('📡 Updating Sheets Config:', logMessage);
    ...
    // ⚡ SECURITY FIX: Don't log config details in production
...
async function writeProducts(sheets, products) {
    ...
    await withRetry(async () => {
        // 1. Update data starting from A1
        ...
        // 2. Clear any leftover rows after the new data
        const nextRow = rows.length + 1;
        await sheets.spreadsheets.values.clear({
            spreadsheetId: sheetsConfig.sheetId,
            range: `Products!A${nextRow}:Q`
        }).catch(err => {
            console.warn('⚠️ [Sheets] Clear leftover rows failed (non-critical):', err.message);
        });
    });
}
